...
Explanation:
  Geom ?geom is fixed, so the in-line constraint
  can only be checked for consistency.


---
PFT entry: (1,0,in-line) (?M_1 is fixed)
---
...
---
PFT entry: (2,0,in-line) (?M_1 is fixed)
---
...
---
PFT entry: (3,0,in-line) (?M_1 is fixed)
---
...
---
PFT entry: (h,h,in-line) (?M_1 is fixed)
---
...
---
PFT entry: (0,1,in-line) (?M_1 is fixed)
---
...
---
PFT entry: (1,1,in-line) (?M_1 is fixed)
---
...
---
PFT entry: (3,1,in-line) (?M_1 is fixed)
---
...
---
PFT entry: (0,2,in-line) (?M_1 is fixed)
---
...
---
PFT entry: (0,3,in-line) (?M_1 is fixed)
---
...
Plan fragment:
  ...
  R[1] = sphere(?point, mag(R[0]));
  R[2] = line(gmp(?M_2), gmz(?M_2));
  R[3] = intersect(R[1], R[4], q_0);
  unless point?(R[3])
    error(perp-dist(R[1], R[2]),
    estring[7]);

  3r/p-p(?geom, ?point, R[3], gmp(?M_1));
  end;
...
Explanation:
  Geom ?geom cannot translate, so the in-line
...
---
PFT entry: (3,3,in-line) (?M_1 is fixed)
---
