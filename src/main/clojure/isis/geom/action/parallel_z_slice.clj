...
Plan fragment:
  begin
  R[0] = line(?m2-point, ?m2-axis);
  R[1] = vec-sum(?m2-point, gmz(?M_1));
  R[2] = vec-sum(?m2-point, gmz(?M_2));
  ...
  R[7] = vec-angle(R[6], R[5], ?m2-axis);

  if null?(?m2-axis_1) and null?(?m2-axis_2)
    then rotate(?m2-link, ?m2-point, ?m2-axis, R[7])
    else 2r/a(?m2-link, ?m2-point, R[7], ?m2-axis, ?m2-axis_1, ?m2-axis_2);
  end;
...
Explanation:
  ...
  Therefore ?m2-link must be rotated about its known point and know axis.
  The z-axes of the two markers must have the same projection onto
  the line defined by ?m2-point and ?m2-axis.
  ...
  (let [ [[m2-link-name m2-proper-name] _] m2
         ...
         axis (ga/line m2-point m2-axis)

         gmz1 (ga/gmz m1 kb)
         gmz2 (ga/gmz m2 kb)

         from-pnt (ga/vec-sum m2-point gmz1)
         to-pnt (ga/vec-sum m2-point gmz2)
