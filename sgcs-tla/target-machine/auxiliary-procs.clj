...
(defn 1t-2r_p-p_point-lf
  ...
  (let [_ (translate ?geom (vec-diff ?to-point ?from-point))
    ...
    (if-not (point? r1)
      ...
      (2r_p-p ?geom ?from-point ?point r1 ?axis-1 ?axis-2))))
...
(defn 1t-2r_p-p_line-lf
  ...
  (let [r0 (vec-diff ?to-point ?from-point)
    ...
    (if-not (point? r5)
      (error (perp-dist r2 r4) estring-7)
      (2r_p-p ?geom ?from-point r5 ?point ?axis-1 ?axis-2))))
...
(defn 1t-3r_p-p_point-lf
  ...
  (let [_ (translate ?geom (vec-diff ?to-point ?from-point))
    ...
    (if-not (point? r1)
      ...
      (3r_p-p ?geom ?from-point ?point r1))))
...
(defn 1t-3r_p-p_line-lf
  ...
  (let [r0 (vec-diff ?to-point ?from-point)
    ...
    (if-not (point? r5)
      (error (perp-dist r2 r4) estring-7)
      (3r_p-p ?geom ?from-point r5 ?point ))))
