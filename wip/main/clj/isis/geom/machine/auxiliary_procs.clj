...
(defn dof-2r:a
  ...
  (let [r0 (circle ?center ?axis 10)
        ...
        r2 (copy r1)
        _ (rotate ?link ?center ?axis (- ?angle))]
    (2rp->p ?link ?center r1 r2 ?axis-1 ?axis-2)))
...
(defn dof-1t-1rp->p:point-lf
  ...
  (let [r0 (vec-diff ?to-point ?from-point)
    ...
    (if-not (point? r4)
      ...
      (1rp->p ?link ?from-point ?point r4 ?axis ?axis-1 ?axis-2))) )


(defn dof-1t-1rp->p:line-lf
  ...
  (let [r0 (vec-diff ?from-point ?to-point)
    ...
    (if-not (point? r6)
      ...
      (dof-1rp->p ?link ?to-point r6 ?point ?axis ?axis-1 ?axis-2))) )
...
(defn dof-1t-1rp->p
  ...
  ((if (equal? ?point ?lf) dof-lt-1rp->p:point-lf  dof-lt-1rp->p:line-lf )
   ?link ?point ?line ?axis ?axis-1 ?axis-2) )


(defn dof-2t-1rp->p:point-lf
  ...
  (let [r0 (vec-diff ?to-point ?from-point)
    ...
    (if-not (point? r4)
      ...
      (dof-1rp->p ?link ?from-point ?point r4 ?axis ?axis-1 ?axis-2))))


(defn dof-2t-1rp->p:plane-lf
  ...
  (let [r0 (vec-diff ?to-point ?from-point)
    ...
    (if-not (point? r5)
      ...
      ;;; ERRATA the ?point below was point?
      (dof-1rp->p ?link ?from-point r5 ?point ?axis ?axis-1 ?axis-2))))
...
(defn dof-2t-1rp->p
  ...
  ((if (equal? ?point ?lf) dof-2t-1rp->p:point-lf  dof-2t-1rp->p:plane-lf )
   ?link ?point ?plane ?axis ?axis-1 ?axis-2 ?from-point ?to-point ?branch) )

(defn dof-1t-2rp->p:point-lf
  "This case is for when ?point is on ?link, and ?line is invariant."
  ...
  (let [_ (translate ?link (vec-diff ?to-point ?from-point))
    ...
    (if-not (point? r1)
      ...
      (dof-2rp->p ?link ?from-point ?point r1 ?axis-1 ?axis-2))))


(defn dof-1t-2rp->p:line-lf
  "This case is for when ?line is on ?link, and ?point is invariant."
  [?link ?point ?line ?axis-1 ?axis-2 ?from-point ?to-point ?branch]
  (let [r0 (vec-diff ?to-point ?from-point)
        r2 (copy ?line)
    ...
    (if-not (point? r5)
      (error (perp-dist r2 r4) estring-7)
      (dof-2rp->p ?link ?from-point r5 ?point ?axis-1 ?axis-2))))
...
(defn dof-1t-2rp->p
  ...
  ((if (equal? ?point ?lf) dof-1t-2rp->p:point-lf  dof-1t-2rp->p:line-lf )
   ?link ?point ?line ?axis-1 ?axis-2 ?from-point ?to-point ?branch) )


(defn dof-1t-3rp->p:point-lf
  "This case is for when ?point is on ?link, and ?line is invariant"
  ...
  (let [_ (translate ?link (vec-diff ?to-point ?from-point))
    ...
    (if-not (point? r1)
      ...
      (dof-3rp->p ?link ?from-point ?point r1))))
;; ERRATA r2 (vec-diff r1 ?from-point)

(defn dof-1t-3rp->p:line-lf
  "This case is for when ?line is on ?link, and ?point is invariant"
  [?link ?point ?line ?from-point ?to-point ?branch]
  ...
  (let [r0 (vec-diff ?to-point ?from-point)
    ...
    (if-not (point? r5)
      (error (perp-dist r2 r4) estring-7)
      (dof-3rp->p ?link ?from-point r5 ?point ))))
...
(defn dof-1t-3rp->p
  ...
  ((if (equal? ?point ?lf) dof-1t-3rp->p:point-lf  dof-1t-3rp->p:line-lf )
   ?link ?point ?line ?from-point ?to-point ?branch) )


(def dof-2t-2rp->p:point-lf
  "This case is for when ?point is on ?link and ?plane is invariant."
  ...
  (let [_ (translate ?link (vec-diff ?to-point ?from-point))
    ...
    (if-not (circle? r1)
      ...
      (dof-2rp->p ?link ?from-point ?point r2 ?axis-1 ?axis-2))))
;; ERRATA r3 (perp-base ?from-point ?plane) r4 (vec-diff r3 ?from-point)

(defn dof-2t-2rp->p:plane-lf
  "This case is for when ?plane is on ?link, and ?point is invariant"
  [?link ?point ?plane ?axis-1 ?axis-2 ?from-point ?to-point]
...
(defn dof-2t-2rp->p
  ...
  ((if (equal? ?point ?lf) dof-2t-2rp->p:point-lf  dof-2t-2rp->p:plane-lf )
   ?link ?point ?line ?from-point ?to-point ?branch) )
